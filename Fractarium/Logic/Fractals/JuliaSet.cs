...
namespace Fractarium.Logic.Fractals
{
	...
	public class JuliaSet : Fractal
	{
		...
		public JuliaSet(BaseParameters parameters, Palette palette, double power, Complex juliaConstant)
		...
		public override int IterateWithExponent(double r, double i)
		{
			...
			for(int iter = 0; iter < Params.IterationLimit; iter++)
			{
				...
				nextI = Math.Pow(r * r + i * i, Power / 2) * Math.Sin(Power * Math.Atan2(i, r)) - J.Imaginary;
				r = nextR;
				i = nextI;
				if(r * r + i * i > DivergenceLimit)
